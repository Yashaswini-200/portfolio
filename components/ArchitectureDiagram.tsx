...
export default function ArchitectureDiagram({ architecture }: ArchitectureDiagramProps) {
  return (
    <section className="rounded border border-border bg-surface p-6">
      <h3 className="font-mono text-base font-semibold text-text-primary">Architecture</h3>
      <div className="mt-6 flex flex-col gap-4 sm:flex-row sm:flex-wrap sm:items-center">
        {architecture.nodes.map((node, index) => (
          <div key={node.id} className="flex items-center gap-3">
            <div className="rounded border border-border bg-surface px-3 py-2 font-mono text-xs text-text-primary">
              {node.label}
            </div>
      ...
      <pre className="mt-6 overflow-x-auto rounded border border-border bg-background px-4 py-3 font-mono text-sm leading-6 text-text-secondary">
